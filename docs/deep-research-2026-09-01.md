...
4. **Query Greenhouse's schema API before launching a browser**, and use its labelled EEOC blocks to enforce the review gate structurally. **Done in two halves, `5f1db60` and `570ff44`.**

    The first half is a cross-check at the submit gate rather than a pre-launch fill. The second closes what that left behind: the gate read nothing but the recruiter's prose, so "structurally" was still an aspiration.

    Measuring it first was the part that mattered. Sampling 38 live Greenhouse postings across 20 boards yielded 40 distinct questions the ATS itself files under `compliance` (EEOC) or `demographic_questions`, and the shipping classifier missed 8. Four causes, none of them guessable: vocabulary the phrase list never had ("transgender", "military status"); an acronym a phrase list cannot chase, since LGBTQ+, LGBTQIA and LGBT2QIA+ are one question spelled three ways; a plural the stemmer mangled, "ethnicities" becoming "ethnicitie" and so never reaching the "ethnicity" rule; and the camelCase labels `DisabilityStatus` and `VeteranStatus`, which tokenize as a single unmatched token. Those 8 were falling through to the generic profile rules, against invariant #2. Re-run against the same 40 captured questions afterwards: 0 missed.

    The structural signal is the last view the gate reads, `BrowserField.metadata["name"]`, which detection already populates and nothing was using here. `disability_status` is what the ATS calls the input, and unlike the label it does not change when a recruiter rewords the question. Camel splitting stays confined to this gate rather than moving into `label_tokens`, because "linkedin" is a live rule that depends on matching a single token and a global split would break it; a test pins that. Every view the gate adds is one-directional, able to pull a question into review but never out of it, which is what makes a wrong guess here survivable.

    Two payload shapes are worth recording, because neither is what the API looks like from memory. `compliance` is a list of `{type, description, questions}`; `demographic_questions` is a dict with its own `questions` array. Both are siblings of `questions`, so `questions_from_payload` never sees either. Folding them in would break `unanswered_required`, which compares `field_names` against the fields actually observed: demographic questions are `required: true` and carry no field names at all, so they would raise a permanent false alarm at every submit gate.
5. **Never ship an ATS score.** Ship parse verification instead. **Half done, `49527c8`.**

    The negative half was already true and stays true. Nothing in the codebase computes a resume-to-job match percentage. The only score-shaped number in the product is `_MATCH_SCORE_FLOOR` in `field_resolution.py`, an internal threshold deciding which profile field answers a form input, shown to nobody.

    The positive half was not merely missing, it was inverted. `mergeIntoProfile` refuses any parsed entry below 0.75 confidence and records it in `skipped`, but no renderer code read that array: `ProfileStore` took `parsedDocument` and `updatedProfile` off the merge summary and threw the rest away. A resume whose roles parsed at 0.74 lost its entire work history on the way in, while the card beside it went on reading PARSED at a document-level confidence that says nothing about which entries were dropped. That is precisely the failure this section warns about, a document that scores well while its work history does not survive extraction, and the app was producing it rather than catching it.

    Onboarding was never exposed to this, because `ConfirmLedger` builds its review from the parsed canonical rather than the merged profile and so already lists the low-confidence rows. `DocumentsScreen` is the only upload path after onboarding and had no verification step at all, which is why the loss was silent there and nowhere else.

    The parse check now sits above the master sources: what landed, what did not and why, what the profile already held, and the parser's own `warnings`, which nothing displayed anywhere before. Merge semantics are deliberately untouched. Making the drop visible and pointing at the profile editor is the fix; lowering the gate would import exactly the entries the parser said it could not read.

    Keyword gap analysis, the second defensible feature named at the top of this section, is not built.
6. **Add a packaged smoke test that exercises a browser adapter.** `scripts/test/packaged-worker-smoke.mjs` currently asserts only `RUN_STARTED`, `JD_ANALYSIS_COMPLETED`, `RESUME_RENDERED`, `PAUSED`. No packaged test touches any browser adapter, which is structurally why a Playwright import bug survived into a shipped build. This gap is the meta-finding of the whole audit.
    ...
    **Update, `d680eaa`/`142aee3`: all three are required now, because all three ship.** The Playwright adapter got a driver -- Patchright, a drop-in fork with the automation tells patched out of the driver rather than papered over from inside the page -- so the reason for its optional status is gone and an absent module means a build that lost its middle fallback. The smoke needed no change: it reads `required` off the report rather than hardcoding a list. The check itself did. Patchright's Python package is a client for a Node driver it ships alongside itself, which reaches the bundle through `--collect-all` and nothing else, so a trimmed build loses the driver while the import keeps succeeding; `driver_check` now follows a successful import with `compute_driver_executable()`, the same function the launch uses to find `node.exe`, and fails on a path that is not there.

    **That last gap is now closed too, `dc2d5a4`.** The offline portal suite runs against `html_replay.py`, a pure-Python reimplementation of the JS `field_detection.py` injects. Nothing verified the twin against a browser, so the whole offline suite could have been a test of itself. `tests/test_browser_fixture_parity.py` serves one Greenhouse-shaped fixture over real HTTP, drives real Chrome through the real adapter, and asserts the three things the offline suite cannot assert about itself: that the fields the twin predicts are exactly the fields the browser finds, that every selector the twin invents resolves to exactly one element in a real document, and that a written answer is really in the DOM when read back through the page rather than through the adapter that wrote it. It is marked `browser`, deselected from the default gate by `-m 'not browser'`, and run from `pnpm test:python:browser`, because it needs a Chrome and opens a window.

    It paid for itself on the first run. The injected script skips `input type="search"` and any control whose name or id matches `recaptcha|captcha|turnstile`; the twin skipped neither, so the offline fixtures were free to assert answers for "Search this site" and "G Recaptcha Response", two questions no user is ever asked. Both are fixed in the twin. The one divergence that cannot be fixed is pinned instead: the twin has no layout engine, so it sees only inline `display:none` on the control itself and stays blind to anything hidden by a stylesheet, a class, or a parent. `test_the_twin_cannot_see_css_and_says_so_here` states that in the suite rather than leaving it to be rediscovered on a live portal.
