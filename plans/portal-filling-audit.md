...
|---|---|---|---|---|---|---|---|
| workday | `portal_registry.py:37` | playwright | `ATS_DIRECT_FORM`, entry actions `portal_workflows.py:41` | none | ideal-HTML replay fixture `tests/test_portal_replay_fixtures.py:23-40` | **No** | Real Workday uses `data-automation-id` ARIA comboboxes, not `select`; discovery at `field_detection.py:91` cannot see them. The repo's own quirk note says so (`portal_workflows.py:158-159`). Also an account-creation wall before the form, unhandled. |
...
| ashby | `portal_registry.py:40` | playwright | `ATS_DIRECT_FORM`, `portal_workflows.py:44` | none | ideal-HTML fixture `tests/test_portal_replay_fixtures.py:74-85` | **No** | Ashby is a React SPA using custom listbox widgets for most non-text answers; not `select`, so invisible. |
...
| 9 | Discover fields across same-origin iframes, open shadow roots, and ARIA widget roles | CRITICAL | L | `browser/field_detection.py`, all three adapters (frame-scoped writes), `browser/adapter.py` | Replay + Playwright tests: saved Greenhouse iframe embed yields its fields; a `role=combobox` renders as a selectable field with its options |
